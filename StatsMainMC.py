...
def main(argv):
    ...
    metadata = pd.read_csv(options.output_path + '/' + options.experiment_name + '/' + 'metadata.csv')
    class_label = metadata.loc[metadata['DATA LABEL'] == 'class label', 'VALUE'].iloc[0]
    instance_label = metadata.loc[metadata['DATA LABEL'] == 'instance label', 'VALUE'].iloc[0]
    sig_cutoff = metadata.loc[metadata['DATA LABEL'] == 'statistical significance cutoff', 'VALUE'].iloc[0]
    cv_partitions = int(metadata.loc[metadata['DATA LABEL'] == 'cv partitions', 'VALUE'].iloc[0])
    do_NB = metadata.loc[metadata['DATA LABEL'] == 'NB', 'VALUE'].iloc[0]
    do_DT = metadata.loc[metadata['DATA LABEL'] == 'DT', 'VALUE'].iloc[0]
    do_RF = metadata.loc[metadata['DATA LABEL'] == 'RF', 'VALUE'].iloc[0]
    do_XGB = metadata.loc[metadata['DATA LABEL'] == 'XGB', 'VALUE'].iloc[0]
...
def submitClusterJob(full_path,encoded_algos,plot_FI_box,class_label,instance_label,experiment_path,cv_partitions,reserved_memory,maximum_memory,queue,plot_metric_boxplots,primary_metric,top_results,sig_cutoff,jupyterRun):
    ...
    sh_file.write('python '+this_file_path+'/StatsJobMC.py '+full_path+" "+encoded_algos+" "+plot_FI_box+" "+class_label+" "+instance_label+" "+str(cv_partitions)+" "+str(plot_metric_boxplots)+" "+str(primary_metric)+" "+str(top_results)+" "+str(sig_cutoff)+" "+str(jupyterRun)+'\n')
    sh_file.close()

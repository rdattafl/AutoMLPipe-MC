...
License: GPL 3.0
Description: Phase 5 of AutoMLPipe-MC - This 'Main' script manages Phase 5 run parameters, updates the metadata file (with user specified run parameters across pipeline run)
             and submits job to run locally (to run serially) or on a linux computing cluster (parallelized).  This script runs ModelJobMC.py which conducts machine learning
             modeling using respective training datasets. This pipeline currently includes the following 10 ML modeling algorithms for multiclass classification:
             * Naive Bayes, Decision Tree, Random Forest, XGBoost, Support Vector Machine (SVM), Artificial Neural Network (ANN),
...
Warnings: Designed to be run following the completion of AutoMLPipe-MC Phase 4 (FeatureSelectionMain.py). SVM modeling should only be applied when data scaling is applied by the pipeline
...
Sample Run Command (Linux cluster parallelized with all default run parameters):
    python ModelMainMC.py --out-path /Users/robert/Desktop/outputs --exp-name myexperiment1
Sample Run Command (Local/serial with with all default run parameters):
    python ModelMainMC.py --out-path /Users/robert/Desktop/outputs --exp-name myexperiment1 --run-parallel False
"""
...
def main(argv):
    ...
    if eval(options.do_all):
        ...
        do_XCS = True
        do_ExSTraCS = True
        algorithms = ['naive_bayes','decision_tree','random_forest','XGB','SVM','ANN','k_neighbors','eLCS','XCS','ExSTraCS']
    ...
    if not options.do_check and not options.do_resubmit: #Run job submission
        ...
        abbrev = {'naive_bayes':'NB','decision_tree':'DT','random_forest':'RF','XGB':'XGB','ANN':'ANN','SVM':'SVM','k_neighbors':'KN','eLCS':'eLCS','XCS':'XCS','ExSTraCS':'ExSTraCS'}
...
def submitLocalJob(algorithm,train_file_path,test_file_path,full_path,n_trials,timeout,lcs_timeout,export_hyper_sweep_plots,instance_label,class_label,random_state,cvCount,filter_poor_features,do_lcs_sweep,nu,iterations,N,training_subsample,use_uniform_FI,primary_metric):
    """ Runs ModelJobMC.py locally, once for each combination of cv dataset (for each original target dataset) and ML modeling algorithm. These runs will be completed serially rather than in parallel. """
    ModelJobMC.job(algorithm,train_file_path,test_file_path,full_path,n_trials,timeout,lcs_timeout,export_hyper_sweep_plots,instance_label,class_label,random_state,cvCount,filter_poor_features,do_lcs_sweep,nu,iterations,N,training_subsample,use_uniform_FI,primary_metric)
...
def submitClusterJob(algorithm,train_file_path,test_file_path,full_path,n_trials,timeout,lcs_timeout,export_hyper_sweep_plots,instance_label,class_label,random_state,experiment_path,cvCount,filter_poor_features,reserved_memory,maximum_memory,do_lcs_sweep,nu,iterations,N,training_subsample,queue,use_uniform_FI,primary_metric):
    """ Runs ModelJobMC.py once for each combination of cv dataset (for each original target dataset) and ML modeling algorithm. Runs in parallel on a linux-based computing cluster that uses an IBM Spectrum LSF for job scheduling."""
    ...
    sh_file.write('python '+this_file_path+'/ModelJobMC.py '+algorithm+" "+train_file_path+" "+test_file_path+" "+full_path+" "+

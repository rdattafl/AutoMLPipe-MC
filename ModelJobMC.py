...
Original file is located at
    https://colab.research.google.com/drive/1fREyMbYHtwvq27c66ZZhxm-nQ6m725o_
"""
...
# Function to generate the line visualization plot from optuna: optuna.visualization.plot_parallel_coordinate

# From ModelJob.py: define the job and runModel functions that calls the helper functions where the modeling will occur.
...
# Now, bring in the dataPrep and saveRuntime functions to prepare the data for modeling and save the runtime of each model.
...
# Next, define the hyper_eval function, which returns the performance of a given hyperparameter trial for a model (using the CV score). 
# This function is used during HP optimization.
...
# These run_{algorithm}_full functions are where the actual modeling occurs.
...
def run_NB_full(x_train, y_train, x_test, y_test,randSeed,i,param_grid,n_trials,timeout,do_plot,full_path,use_uniform_FI,primary_metric):
    ...
    probas_ = model.predict_proba(x_test) # .predict_proba() - A method in classifiers and clusterers that can return probability 
                                          # estimates for each class/cluster. Its input is usually only some observed data, X.
    ...
    # As a refresher, these metrics will be Balanced Accuracy score, a Confusion Matrix, an roc_auc_score, and Matthew's Correlation Coefficient.
...
def run_DT_full(x_train, y_train, x_test, y_test,randSeed,i,param_grid,n_trials,timeout,do_plot,full_path,use_uniform_FI,primary_metric):
    ...
    # Relevant MC classification metrics (Balanced Accuracy, Confusion Matrix, and Top-k Accuracy Score)
...
# This section of code will define all the hyperparameters that will be swept through and optimized when the study.optimize() and 
# hyper_eval() methods are called. It also contains the export_best_params function, which stores the optimal hyperparameters for 
# a given model into a CSV file.
...
def hyperparameters(random_state,do_lcs_sweep,nu,iterations,N):
    ...
    param_grid_SVM = {'kernel': ['linear', 'poly','rbf'],'C': [0.1, 1000],'gamma': ['scale'],'degree': [1, 6], # LinearSVC only has implementation for linear kernels.

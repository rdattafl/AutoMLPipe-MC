...
License: GPL 3.0
Description: Phase 5 of AutoMLPipe-MC - This 'Job' script is called by ModelMain.py and runs machine learning modeling using respective training datasets.
            This pipeline currently includes the following 10 ML modeling algorithms for multiclass classification:
...
def runModel(algorithm,train_file_path,test_file_path,full_path,n_trials,timeout,lcs_timeout,export_hyper_sweep_plots,instance_label,class_label,random_state,cvCount,filter_poor_features,do_lcs_sweep,nu,iterations,N,training_subsample,use_uniform_FI,primary_metric,param_grid):
    ...
    abbrev = {'naive_bayes':'NB','decision_tree':'DT','random_forest':'RF','XGB':'XGB','SVM':'SVM','ANN':'ANN','k_neighbors':'KN','eLCS':'eLCS','XCS':'XCS','ExSTraCS':'ExSTraCS'}
...
def run_NB_full(x_train, y_train, x_test, y_test,randSeed,i,param_grid,n_trials,timeout,do_plot,full_path,use_uniform_FI,primary_metric):
    ...
    # Now, we want to implement relevant multi-class classification metrics for all the algorithms that will be used in this modified pipeline.
    # As a refresher, these metrics will be Balanced Accuracy score, a Confusion Matrix (and its plot), an roc_auc_score, and Matthew's Correlation Coefficient.
    bac = balanced_accuracy_score(y_test, y_pred)
    conf = confusion_matrix(y_test, y_pred) 
    ras = roc_auc_score(y_test, probas_, multi_class='ovr')
    mcc = matthews_corrcoef(y_test, y_pred)
    conf_plot = plot_confusion_matrix(model, x_test, y_test, normalize='all')
    ...
    return [bac, conf, ras, mcc, conf_plot, fi]
...
def run_DT_full(x_train, y_train, x_test, y_test,randSeed,i,param_grid,n_trials,timeout,do_plot,full_path,use_uniform_FI,primary_metric):
    ...
    print(clf) #Print basic classifier info/hyperparameters for verification
    ...
    # Determine probabilities of class predictions for each test instance
    probas_ = model.predict_proba(x_test)
    # Relevant MC classification metrics (Balanced Accuracy, Confusion Matrix, ROC-AUC Score, and Matthew's Correlation Coefficient)
    bac = balanced_accuracy_score(y_test, y_pred)
    conf = confusion_matrix(y_test, y_pred) 
    ras = roc_auc_score(y_test, probas_, multi_class='ovr')
    mcc = matthews_corrcoef(y_test, y_pred)
